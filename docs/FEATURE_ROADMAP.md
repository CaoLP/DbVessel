...
- [x] **Phân trang server-side khi xem dữ liệu bảng** ([PaginationBar.tsx](../apps/desktop/src/components/PaginationBar.tsx)): double-click bảng / "Xem dữ liệu" giờ dùng `LIMIT/OFFSET` thật + `SELECT COUNT(*)` để biết tổng số dòng, hiện thanh phân trang dưới grid (chọn số dòng/trang, Trang X/Y, dòng A–B/tổng). Chỉ áp dụng cho luồng browse bảng tiện lợi (double-click/menu), không tự ý phân trang câu query người dùng tự gõ (không có parser SQL thật để bọc COUNT/LIMIT an toàn cho query bất kỳ).
